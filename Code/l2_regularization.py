...
for i in range(1,noPoly+1):
    ...
    ridge_regr = Ridge()
    cv_grid_scores_mean,cv_grid_scores_std = grid_search_helper(X, Y_train,ridge_regr, param_grid,gridSize, n_jobs=-1,
    ...
    ridge_regr= Ridge(alpha=grid_values[0,opt_ind])
    ridge_regr.fit(X,Y_train)
    print ridge_regr.score(X,Y_train)
    if(i>1):
    ...
    r2Poly[i-1]=ridge_regr.score(X_val_tr,Y_val)
